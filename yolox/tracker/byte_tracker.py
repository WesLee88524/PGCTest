...
class STrack(BaseTrack):
    ...
    def virtual_update(self, pred_tlwh, frame_id, score):
        """Keep a track alive with a learned or heuristic PGC correction."""
        self.frame_id = frame_id
        self.tracklet_len += 1
...
class BYTETracker(object):
    def __init__(self, args, frame_rate=30, oracle_injector=None, gt_box_provider=None):
        ...
        self.use_pgc = bool(getattr(args, "use_pgc", True))
        self.use_pgc_pair = bool(getattr(args, "use_pgc_pair", True))
        self.use_pgc_delta = bool(getattr(args, "use_pgc_delta", True))
        self.use_pgc_virtual = bool(getattr(args, "use_pgc_virtual", False))
    ...
    def update(self, output_results, img_info, img_size, gt_boxes=None):
        ...
        if self.use_pgc and self.use_pgc_pair and self.use_pgc_low_relax:
            dists = self._apply_pgc_low_conf_relaxation(dists, r_tracked_stracks)
        ...
        for it in u_track:
            ...
            if self.use_pgc and self.use_pgc_pair:
                track.mark_pgc_unmatched()
            if (
                self.use_pgc
                and self.use_pgc_pair
                and self.use_pgc_virtual
                and self.allow_pgc_virtual_output
                and self._pgc_virtual_maintenance(track)
            ):
                activated_starcks.append(track)
                continue
    ...
    def _association_distance(self, tracks, detections):
        ...
        if self.use_pgc and self.use_pgc_pair and self.use_pgc_delta and self.use_pgc_pred_assoc:
            ...
            center_cost = self._normalized_center_distance(track_tlwhs, [det.tlwh for det in detections])
            return 0.82 * iou_cost + 0.18 * center_cost
        return matching.iou_distance(tracks, detections)

    @staticmethod
    def _normalized_center_distance(track_tlwhs, det_tlwhs):
    ...
    def _apply_pgc_low_conf_relaxation(self, cost_matrix, tracks):
        if cost_matrix.size == 0:
            return cost_matrix
        relaxed = cost_matrix.copy()
        for row, track in enumerate(tracks):
            factor = 1.0 - float(self.pgc_low_beta) * float(getattr(track, "pgc_occlusion", 0.0)) * float(
                getattr(track, "pgc_group_reliability", 0.0)
            )
