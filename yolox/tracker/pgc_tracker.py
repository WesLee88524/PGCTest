...
class PGCRelationManager(object):
    """Pair modeling for ablation-friendly online inference.

    If a checkpoint is provided, the learned branch predicts the delta box
    correction. Otherwise we keep a heuristic fallback so the component can be
    enabled independently for ablations.
    """
    ...
    def __init__(self, args=None):
        ...
        self.tau_react = getattr(args, "pgc_tau_react", 0.60)
        self.tau_assoc = getattr(args, "pgc_tau_assoc", 0.35)
        self.k_on = getattr(args, "pgc_k_on", 5)
        self.k_off = getattr(args, "pgc_k_off", 3)
        self.k_max = getattr(args, "pgc_k_max", 3)
    ...
    def _apply_learned_group_context(self, tracks, track_by_id, frame_id):
        ...
        if not active_tracks or self.model is None:
            return
